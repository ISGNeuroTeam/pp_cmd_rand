...
all:
 ...
 test - run all tests\n\
 pack - make output archive, file name format \"rand_vX.Y.Z_BRANCHNAME.tar.gz\"\n\
"
...
pack: make_build
	rm -f rand-*.tar.gz
	echo Create archive \"rand-$(VERSION)-$(BRANCH).tar.gz\"
	cd make_build; tar czf ../rand-$(VERSION)-$(BRANCH).tar.gz rand
...
rand.tar.gz: build
	cd make_build; tar czf ../rand.tar.gz rand && rm -rf ../make_build
