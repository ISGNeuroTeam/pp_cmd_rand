...
all:
 ...
 test - run all tests\n\
 pack - make output archive, file name format \"pp_cmd_rand_vX.Y.Z_BRANCHNAME.tar.gz\"\n\
"
...
pack: make_build
	rm -f pp_cmd_rand-*.tar.gz
	echo Create archive \"pp_cmd_rand-$(VERSION)-$(BRANCH).tar.gz\"
	cd make_build; tar czf ../pp_cmd_rand-$(VERSION)-$(BRANCH).tar.gz rand

clean_pack:
	rm -f pp_cmd_rand-*.tar.gz


pp_cmd_rand.tar.gz: build
	cd make_build; tar czf ../pp_cmd_rand.tar.gz rand && rm -rf ../make_build
...
make_build:
	...
	echo make_build
	mkdir make_build
	cp -R ./rand make_build
	cp *.md make_build/rand/
...
dev: venv
	rm -rf ./venv/lib/python3.9/site-packages/postprocessing_sdk/pp_cmd/rand
...
test:
	docker build --progress=plain -t pp_cmd_rand:$(VERSION) . && docker run -v $$(pwd):/app -it pp_cmd_rand:$(VERSION) python -m unittest tests/test_command.py
